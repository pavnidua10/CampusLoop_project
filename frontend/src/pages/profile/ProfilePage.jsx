...
const ProfilePage = () => {
	...
	const { user: loggedInUser } = useAuth(); // ✅ destructured user object from auth context
	...
	const isSubscribed = (user?.followers ?? []).includes(loggedInUser?._id);
	...
	onClick={async () => {
		try {
			...
			navigate(`/chat/${data._id}`, {
				...
				console.log("Accessed chat ID:", data._id);
	...
	className='btn btn-primary rounded-full btn-sm text-white ml-2 px-4'
>
	Message
</button>
