...
const MenteeChat = ({ mentee, chatId }) => {
  ...
  // 🔹 Listen for incoming mentorship messages
  useEffect(() => {
    const handleReceiveMessage = (newMsg) => {
      const formattedMsg = {
        ...newMsg,
    ...
    const fetchMessages = async () => {
      try {
        const res = await fetch(
          `${API_URL}/api/mentorship-chats/messages/${chatId}`,{
          credentials: "include"}
        );
        const data = await res.json();
  ...
  return (
    <div className="flex flex-col h-full p-1 bg-white">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-2 bg-white rounded-2xl border shadow-lg h-[500px]">
        {messages.length === 0 ? (
          <p className="text-center text-gray-400">No messages yet</p>
        ) : (
          messages.map((msg, idx) => (
